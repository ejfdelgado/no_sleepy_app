...
class MainActivity : AppCompatActivity() {
    ...
    private fun startGpsService() {
        ...
        // If data is loaded, evaluate state. Otherwise, just boot service without pausing.
        if (adapter.getItems().isNotEmpty()) {
            val anyEnabled = adapter.getItems().any { it.enabled }
            intent.action = if (anyEnabled) GpsTrackerService.ACTION_RESUME_TRACKING else GpsTrackerService.ACTION_PAUSE_TRACKING
        } else {
            intent.action = GpsTrackerService.ACTION_RESUME_TRACKING
        }
